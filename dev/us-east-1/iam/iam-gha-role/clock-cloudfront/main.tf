  ...
  role_policy = jsonencode({
    ...
    Statement = [
      ...
      # 2) tag read for your specific group (correct action name)
      {
        ...
        Resource = "arn:aws:logs:${local.region}:${local.account_id}:log-group:/ecs/clock-cloudfront-dev"
      },
      ...
      # List bucket for your ECS state prefix
      {
        ...
        Condition = {
          StringLike = { "s3:prefix" = [
            "dev/us-east-1/ecs/clock-cloudfront/*",
            "dev/us-east-1/ecs/clock-cloudfront"
          ] }
        }
      },
      # R/W on ECS state objects
      {
        ...
        Resource = "arn:aws:s3:::sae-s3-terraform-backend/dev/us-east-1/ecs/clock-cloudfront/*"
      },
      # List bucket for the remote states you READ
      {
        ...
        Condition = {
          StringLike = { "s3:prefix" = [
            "dev/us-east-1/vpc/*",
            "dev/us-east-1/vpc",
            "dev/us-east-1/ecr/clock-cloudfront/*",
            "dev/us-east-1/ecr/clock-cloudfront",
      ...
      # Read-only on those remote state objects
      {
        ...
        Resource = [
          ...
          "arn:aws:s3:::sae-s3-terraform-backend/dev/us-east-1/ecr/clock-cloudfront/*",
